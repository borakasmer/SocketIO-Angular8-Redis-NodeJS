...
app.get("/people", token, function (req, res) {
    ...
    if (redisClient.connected) {
        ...
        redisClient.keys('User*', function (err, keys) {
            ...
            if (keys.length > 0) {//Redis'de kayıt var ise ordan gelir.
                var personList = [];
                for (var i = 0, len = keys.length; i < len; i++) {
                    ...
                    redisClient.get(keys[i], function (err, user) {
                        ...
                        if (personList.length == keys.length - 1) {
                            res.send(personList);
                ...
                //Read All Data From MongoDB. Redis'de kayıt yok.
                var counter = 0;
                Human.find(function (err, doc) {
                    doc.forEach(function (item) {
                        ...
                        var userName = 'User:' + item._doc.username;
                        redisClient.get(userName, function (err, user) {
                            if (user == null) {
                                ...
                                console.log(JSON.parse(user).name.first);
                            }
                        });
                        //End Redis Write
                    });
    ...
    else //Redis Connect Değil ise mongoDB'den çekilir.
    {
        //var counter = 0;
        Human.find(function (err, doc) {
            doc.forEach(function (item) {
...
app.post('/insertPeople', token, async (req, res) => {
    ...
    try {
        ...
        //Redis Insert
        if (redisClient.connected) {
            var userName = 'User:' + req.body.username;
...
app.post('/updatePeople', token, async (req, res) => {
    ...
    try {
        ...
        //Redis Update
        if (redisClient.connected) {
            var userName = 'User:' + updatePerson.username;
            var data = JSON.stringify(updatePerson);
            redisClient.set(userName, data, function (err, res) { });
        }
        //--------------
