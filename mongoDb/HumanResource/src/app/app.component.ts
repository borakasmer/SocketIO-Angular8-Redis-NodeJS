...
export class AppComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.service.updatedPerson.subscribe((person: Person) => {
      debugger;
      let updateIndex = this.peopleList.findIndex(per => per.username === person.username)
      this.peopleList[updateIndex] = person;
      //alert(person.name.first + " " + person.name.last);
    });

    this.service.savedPerson.subscribe((person: Person) => {
      debugger;
      this.peopleList.unshift(person);
    });

    this.service.deletedPerson.subscribe((person: Person) => {
      debugger;
      let deleteIndex = this.peopleList.findIndex(per => per.username === person.username)
      this.peopleList.splice(deleteIndex, 1);
    });
