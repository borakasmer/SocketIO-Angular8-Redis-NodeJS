...
export class PersonService {
    ...
    sendUpdatePerson(person: Person) {
        this.socket.emit('sendUpdatePerson', person);
    }
    sendSavePerson(person: Person) {
        this.socket.emit('sendSavePerson', person);
    }
    sendDeletePerson(person: Person) {
        debugger;
        this.socket.emit('sendDeletePerson', person);
    }

    updatedPerson = this.socket.fromEvent<Person>('Updatedperson');
    savedPerson = this.socket.fromEvent<Person>('Savedperson');
    deletedPerson = this.socket.fromEvent<Person>('Deletedperson');
        ...
        return this.httpClient.post<any>(this.updateUrl, JSON.stringify(data), httpOptions)
            .pipe(
                map(response => {
                    ...
                    this.sendUpdatePerson(data); // Send SocketIO
                    return response.body;
                }),
                retry(1),
                catchError(this.errorHandel)
        ...
        return this.httpClient.post<any>(this.inserteUrl, JSON.stringify(data), httpOptions)
            .pipe(
                map(response => {
                    ...
                    //YENİ EKLENEN SATIR
                    //this.sendSavePerson(data); //Send SocketIO
                    this.sendSavePerson(response.body); //Send SocketIO
                    //-------------------
                    return response.body;
                }),
                retry(1),
        ...
        return this.httpClient.post<any>(this.deleteUrl, JSON.stringify(data), httpOptions)
            .pipe(
                map(response => {
                    ...
                    //YENİ EKLENEN SATIR               
                    this.sendDeletePerson(data); // Send SocketIO
                    //-------------------
                    return response.body;
                }),
                retry(1),
